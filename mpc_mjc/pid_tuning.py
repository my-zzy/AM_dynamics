...
Usage (from workspace root, conda activate main):
    python mpc_mjc/pid_tuning.py --mode p2p
    python mpc_mjc/pid_tuning.py --mode figure8

Tune gains via the constants at the top of this file.
Arm joints are locked at zero throughout.
"""
import sys
import os
import time
...
# ===========================================================================
#  Arm joint PD (holds joints at zero)
# ===========================================================================
KP_ARM = 3.0
KD_ARM = 0.3
...
def fix_arm(data, model):
    """PD torque to keep arm joints at zero."""
    j1 = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, 'joint1')
    j2 = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, 'joint2')
    for ctrl_idx, jnt_id in ((ARM_CTRL_J1, j1), (ARM_CTRL_J2, j2)):
        q   = data.qpos[model.jnt_qposadr[jnt_id]]
        qd  = data.qvel[model.jnt_dofadr[jnt_id]]
        tau = np.clip(-KP_ARM * q - KD_ARM * qd, -5.0, 5.0)
